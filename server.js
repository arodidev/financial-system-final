...
server.use((req, res, next) => { //app.use() executes for all requests coming from the client 
...
//checks jwt
const requireAuth = (req, res, next) => { //this middleware has to be used for specific routes, not all routes hence no server.use
    ...
    if (token) {
        jwt.verify(token, 'jamie arodi secret', async (err, decodedToken) => {
            if (err) {
                ...
                console.log(decodedToken);
                let user = await dataModel.findById(decodedToken.id);
...
server.post('/signup', async (req, res) => {
    ...
    try {
        const userA = await dataModel.create({ firstName, lastName, email, password });
...
server.post('/loans', async (req, res) => {
    ...
    const {email, totalCont, loanAmount} = toChangeData;
        ...
        const docA = await dataModel.findOneAndUpdate(
...
server.post('/contribution', async (req, res) => {
    const docB = await dataModel.findOneAndUpdate(
        { email: req.body.email },
        {
            '$set':
            {
                monthlyCont: req.body.monthlyCont,
                totalCont: parseInt(req.body.totalCont) + parseInt(req.body.monthlyCont), 
                new: true
            }
        })
        .then(result => {
...
const handleErrors = (err) => {
    ...
    if (err.code === 11000) {
        errors.errorss.email = "That email has already been used"
    ...
    if (err.message === 'incorrect email or password') {
        errors.errorss.email = "incorrect email or password";
    }

    //validation errors
    if (err.message.includes('Userdata validation failed')) {
        Object.values(err.errors).forEach((error) => {
            errors.errorss[error.properties.path] = error.properties.message;
        });
    }
    return errors;
}
